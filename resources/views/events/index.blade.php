                    ...
                    @foreach ($events as $event)
                        <tr onclick="window.location='{{ route('events.show', $event->id) }}';" style="cursor:pointer;">
                            ...
                            @if ($event->availability->available != 0)
                                <td><a href=""><x-icon icon="ticket" />Koop tickets</a></td>
                            @else
                                ...
                                @if (Auth::user()->is_admin)
                                    ...
                                    <td>
                                        <form action="{{ route('events.destroy', $event->id) }}" method="Post">
                                            <a class="btn btn-primary" href="{{ route('events.edit', $event->id) }}"><x-icon icon="pen-to-square" />Bewerk</a>
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger"><x-icon icon="trash-can" />Verwijder</button>
                                        </form>
                                    </td>
                                @endif
                            @endauth
                            <td><x-icon icon="arrow-up-right-from-square" /></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
